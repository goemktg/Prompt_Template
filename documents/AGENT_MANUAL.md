...
순수 대화형 작업(3문장 이하의 경량 사실 답변)이 아닌 모든 실질적 작업에 적용합니다.

단계 정의 전에 `.github/copilot-instructions.md`의 `0-SKILL → 0-INTENT → 0-GATE` 순서를 적용하여, 오케스트레이터가 이미 호출된 실행 문맥을 기본 전제로 두고 현재 실행 문맥이 경량 직접 응답 carveout을 유지할지, 아니면 오케스트레이터 인보크 기반 위임 흐름으로 진행할지를 먼저 결정합니다:

- **스킬 우선 확인 (`0-SKILL`)**: 등록된 스킬이 있으면 먼저 매칭하되, 프롬프트 에셋/문서/소스 코드 같은 산출물 유형과 충돌하는 일반 키워드 매치는 `0-INTENT` 우선 규칙으로 재판정합니다.
- **Tier 0/1 스킬 단락 규칙**: Tier 0/1로 확정된 스킬은 `0-GATE`를 건너뛰고 현재 실행 문맥에서 직접 실행합니다. 다만 산출물 우선순위와 충돌하는 모호한 일반 키워드 매치는 예외적으로 `0-INTENT` 재분류를 거칩니다.
- **의도 분류 (`0-INTENT`)**: 스킬이 없거나 일반 키워드가 모호하면, 산출물 유형과 도메인 우선순위로 적절한 전문 에이전트를 결정합니다.
- **위임 필요 여부 결정 (`0-GATE`)**: 위 분류 결과를 바탕으로 경량 직접 응답 carveout 유지 여부 또는 위임 실행 필요 여부를 결정합니다.
- **직접 응답 유지**: `0-GATE`가 모두 NO이면 현재 실행 문맥은 경량 직접 응답만 수행합니다.
- **단순 위임**: `0-GATE`에 YES가 하나라도 있으면 현재 오케스트레이터 인보크 문맥에서 `AGENTS.md`를 참고하여 직접 적절한 전문 에이전트에 위임합니다.
- **복잡 위임**: 3개 이상 에이전트 호출, 명시적 의존성, 교차 도메인 산출물이 필요한 경우에만 추가 planning support로 `@orchestrator`를 사용합니다.
- **오케스트레이터 역할**: 오케스트레이터는 기본 인보크 문맥이며, 추가 호출 시에는 범용 라우터가 아니라 순서/의존성 계획자로 동작합니다. 에이전트 카탈로그 자체를 대체하지 않습니다.

**추가 `@orchestrator` planning support 사용 시 TODO 처리 (중요)**:

- `@orchestrator` 가 반환되면 TODO 리스트는 **오케스트레이터가 이미 생성**한 상태입니다.
- 각 TODO 제목은 `@agent-name: 간략한 작업 설명` 형식입니다.
- 현재 실행 문맥은 **TODO를 중복 생성하지 않습니다**. 오케스트레이터가 만든 리스트를 그대로 실행합니다.
...
2. **Phase 2. CONFIRM**: **현재 오케스트레이터 인보크 문맥이 담당**. 보고서 경로를 노출하고 명시적 사용자 승인(Explicit Approval)을 요청. (계획 반려 시 최대 3회까지 Phase 1 반복 수정 루프 수행)
...
1. **`0-SKILL → 0-INTENT → 0-GATE` 실행** (`.github/copilot-instructions.md` Agent Interaction Protocol 참조)
   - 등록 스킬 확인 → 모호 시 의도 재분류 → G1–G3 평가
   - Tier 0/1 스킬 확정 시 `0-GATE` 생략 후 사전 확정된 문맥에서 직접 실행
   - G1–G3 중 하나라도 YES → 오케스트레이터 인보크 기반 위임 경로 확정 및 적절한 `runSubagent` 경로 선택
   - 모두 NO → 직접 답변 허용 (≤3문장)
...
- [ ] **`0-SKILL → 0-INTENT → 0-GATE` 실행** (`.github/copilot-instructions.md`) — 오케스트레이터 인보크 기반 문맥에서 직접 응답 carveout 여부와 위임 경로를 먼저 결정
