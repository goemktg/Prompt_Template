...
import write_lifecycle_transition
...
class LifecycleRuntimeContractTests(unittest.TestCase):
    ...
    def test_hook_manifest_references_expected_scripts(self) -> None:
        ...
        self.assertEqual(set(hooks), {"sessionStart", "sessionEnd", "preCompact"})
    ...
    def test_session_hooks_update_lifecycle_state_for_workspace_payload(self) -> None:
        session_init = REPO_ROOT / "copilot" / "hooks" / "scripts" / "session_init.py"
        session_end = REPO_ROOT / "copilot" / "hooks" / "scripts" / "session_end.py"
        pre_compact = REPO_ROOT / "copilot" / "hooks" / "pre-compact.py"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            init_result = self._run_hook(session_init, payload)
            self.assertEqual(init_result.returncode, 0, init_result.stderr)

            started_state = self._read_state_file(workspace_root)
            self.assertEqual(started_state["lifecycle_state"]["current_phase"], "INIT")
            self.assertEqual(started_state["lifecycle_state"]["status"], "active")
            self.assertEqual(started_state["lifecycle_state"]["active_task"], "implement lifecycle hooks")
            self.assertEqual(
                started_state["lifecycle_state"]["continuity"]["last_event"],
                "sessionStart",
            )
            self.assertEqual(
                started_state["lifecycle_state"]["continuity"]["workspace_root"],
                str(workspace_root.resolve()),
            )

            compact_result = self._run_hook(pre_compact, payload)
            self.assertEqual(compact_result.returncode, 0, compact_result.stderr)

            compacted_state = self._read_state_file(workspace_root)
            self.assertTrue(compacted_state["lifecycle_state"]["continuity"]["compact_safe"])
            self.assertTrue(
                compacted_state["lifecycle_state"]["continuity"]["last_compaction_checkpoint"]
            )
            self.assertEqual(compacted_state["lifecycle_state"]["status"], "active")

            end_result = self._run_hook(session_end, payload)
            self.assertEqual(end_result.returncode, 0, end_result.stderr)

            ended_state = self._read_state_file(workspace_root)
            self.assertEqual(ended_state["lifecycle_state"]["current_phase"], "FINALIZE")
            self.assertEqual(ended_state["lifecycle_state"]["status"], "idle")
            self.assertIsNone(ended_state["lifecycle_state"]["active_task"])
            self.assertIsNone(ended_state["lifecycle_state"]["current_plan_hash"])
            self.assertEqual(
                ended_state["lifecycle_state"]["continuity"]["last_event"],
                "sessionEnd",
            )
            self.assertTrue(ended_state["lifecycle_state"]["continuity"]["last_session_end"])

    def test_session_start_hydrates_existing_await_state(self) -> None:
        session_init = REPO_ROOT / "copilot" / "hooks" / "scripts" / "session_init.py"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            store.update_lifecycle_state(
                ...
                current_phase="AWAIT",
                current_plan_hash="plan-await-123",
                status="awaiting",
            )
            ...
            payload = {
                ...
                "timestamp": 1_713_456_789_000,
                "source": "unit-test",
                "sessionId": "session-await-123",
                "task": "should not overwrite await state",
            }

            init_result = self._run_hook(session_init, payload)
            self.assertEqual(init_result.returncode, 0, init_result.stderr)

            hydrated_state = self._read_state_file(workspace_root)["lifecycle_state"]
            self.assertEqual(hydrated_state["current_phase"], "AWAIT")
            self.assertEqual(hydrated_state["status"], "awaiting")
            self.assertEqual(hydrated_state["active_task"], "wait for approval")
            self.assertIs(hydrated_state["approval_pending"], True)
            self.assertEqual(
                hydrated_state["await_context"],
                {
                    "approval_pending": True,
                    "next_transition": "EXECUTE",
                    "reason": "Need approval before EXECUTE",
            ...
            self.assertEqual(hydrated_state["current_plan_hash"], "plan-await-123")
            self.assertEqual(
                hydrated_state["continuity"]["next_transition"],
                "EXECUTE",
            )
            self.assertEqual(
                hydrated_state["continuity"]["resume_token"],
                "resume-123",
            )
            self.assertTrue(
                hydrated_state["continuity"]["hydrated_from_previous_session"]
            )
            self.assertEqual(
                hydrated_state["continuity"]["last_event"],
                "sessionStart",
            )
            self.assertTrue(hydrated_state["continuity"]["last_session_start"])
            self.assertFalse(hydrated_state["continuity"]["compact_safe"])
            self.assertNotIn("last_compaction_checkpoint", hydrated_state["continuity"])
            self.assertNotIn("last_session_end", hydrated_state["continuity"])
            self.assertNotIn("last_terminal_status", hydrated_state["continuity"])
            self.assertNotIn("legacy_marker", hydrated_state["continuity"])

    def test_session_start_overwrites_non_await_state_with_init(self) -> None:
        session_init = REPO_ROOT / "copilot" / "hooks" / "scripts" / "session_init.py"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            store = upgrade_state.UpgradeStateStore(workspace_root)
            store.update_lifecycle_state(
                active_task="stale execute task",
                approval_pending=False,
                await_context={"reason": "stale"},
                continuity={
                ...
                current_phase="PLAN",
                current_plan_hash="stale-plan-hash",
                status="active",
            )

            payload = {
                "workspacePath": str(workspace_root),
                "task": "fresh session init task",
            }

            init_result = self._run_hook(session_init, payload)
            self.assertEqual(init_result.returncode, 0, init_result.stderr)

            started_state = self._read_state_file(workspace_root)["lifecycle_state"]
            self.assertEqual(started_state["current_phase"], "INIT")
            self.assertEqual(started_state["status"], "active")
            self.assertEqual(started_state["active_task"], "fresh session init task")
            self.assertIs(started_state["approval_pending"], False)
            self.assertIsNone(started_state["await_context"])
            self.assertIsNone(started_state["current_plan_hash"])
            self.assertEqual(started_state["continuity"]["last_event"], "sessionStart")
            self.assertTrue(started_state["continuity"]["last_session_start"])
            self.assertFalse(started_state["continuity"]["compact_safe"])
            self.assertNotIn("resume_token", started_state["continuity"])
            self.assertNotIn("last_session_end", started_state["continuity"])
            self.assertNotIn("legacy_marker", started_state["continuity"])
            self.assertNotIn("last_compaction_checkpoint", started_state["continuity"])
            self.assertNotIn("hydrated_from_previous_session", started_state["continuity"])

    def test_session_start_overwrites_finalized_state_with_init_and_clears_terminal_residue(self) -> None:
        session_init = REPO_ROOT / "copilot" / "hooks" / "scripts" / "session_init.py"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            store.update_lifecycle_state(
                ...
                await_context=None,
                continuity={
                    "hydrated_from_previous_session": True,
                    "last_session_end": True,
                    "last_terminal_status": "completed",
                    "legacy_marker": "terminal-residue",
                    "next_transition": "EXECUTE",
                    "resume_token": "stale-terminal-token",
                    "workspace_root": str(workspace_root.resolve()),
                },
                current_phase="FINALIZE",
                current_plan_hash="stale-terminal-plan-hash",
                status="idle",
            )

            payload = {
                "workspacePath": str(workspace_root),
                "task": "fresh session after finalize",
            }
            ...
            self.assertEqual(started_state["current_phase"], "INIT")
            self.assertEqual(started_state["status"], "active")
            self.assertEqual(started_state["active_task"], "fresh session after finalize")
            self.assertIs(started_state["approval_pending"], False)
            self.assertIsNone(started_state["await_context"])
            self.assertIsNone(started_state["current_plan_hash"])
            self.assertEqual(started_state["continuity"]["last_event"], "sessionStart")
            ...
            self.assertFalse(started_state["continuity"]["compact_safe"])
            self.assertEqual(
                started_state["continuity"]["workspace_root"],
                str(workspace_root.resolve()),
            )
            self.assertNotIn("hydrated_from_previous_session", started_state["continuity"])
            self.assertNotIn("last_session_end", started_state["continuity"])
            self.assertNotIn("last_terminal_status", started_state["continuity"])
            self.assertNotIn("legacy_marker", started_state["continuity"])
            self.assertNotIn("next_transition", started_state["continuity"])
            self.assertNotIn("resume_token", started_state["continuity"])

    def test_write_lifecycle_transition_writes_plan_state(self) -> None:
        writer_script = REPO_ROOT / "scripts" / "write_lifecycle_transition.py"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(stored["lifecycle_state"]["current_phase"], "PLAN")
            self.assertEqual(stored["lifecycle_state"]["status"], "active")
            self.assertEqual(stored["lifecycle_state"]["active_task"], "draft lifecycle slices")
            self.assertEqual(stored["lifecycle_state"]["current_plan_hash"], "plan-abc123")
            self.assertEqual(
                stored["lifecycle_state"]["continuity"]["workspace_root"],
                str(workspace_root.resolve()),
            )

    def test_write_lifecycle_transition_writes_await_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            workspace_root = Path(tmpdir)

            state = write_lifecycle_transition.write_lifecycle_transition(
                workspace_root=workspace_root,
                phase="AWAIT",
                status="awaiting",
                active_task="wait for approval",
                approval_pending=True,
                await_reason="Need explicit user approval before EXECUTE",
                next_transition="EXECUTE",
            )

            lifecycle_state = state["lifecycle_state"]
            self.assertEqual(lifecycle_state["current_phase"], "AWAIT")
            self.assertEqual(lifecycle_state["status"], "awaiting")
            self.assertEqual(lifecycle_state["active_task"], "wait for approval")
            self.assertIs(lifecycle_state["approval_pending"], True)
            self.assertEqual(
            ...
            self.assertEqual(lifecycle_state["continuity"]["next_transition"], "EXECUTE")

    def test_write_lifecycle_transition_await_preserves_resume_adjacent_fields(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            workspace_root = Path(tmpdir)

            write_lifecycle_transition.write_lifecycle_transition(
                workspace_root=workspace_root,
                phase="PLAN",
                status="active",
                active_task="resume from await",
                plan_hash="plan-await-001",
            )
            upgrade_state.UpgradeStateStore(workspace_root).update_lifecycle_state(
                continuity={
                    "resume_token": "plan-await-001",
                    "workspace_root": str(workspace_root.resolve()),
                },
            )
            ...
            state = write_lifecycle_transition.write_lifecycle_transition(
                ...
                status="awaiting",
                approval_pending=True,
                await_reason="Need approval before EXECUTE",
                next_transition="EXECUTE",
            )
            ...
            self.assertEqual(lifecycle_state["current_phase"], "AWAIT")
            self.assertEqual(lifecycle_state["status"], "awaiting")
            self.assertEqual(lifecycle_state["active_task"], "resume from await")
            self.assertEqual(lifecycle_state["current_plan_hash"], "plan-await-001")
            self.assertEqual(
                lifecycle_state["await_context"],
                {
                    "approval_pending": True,
    ...
    def test_write_lifecycle_transition_leaving_await_clears_resume_only_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            workspace_root = Path(tmpdir)
            store = upgrade_state.UpgradeStateStore(workspace_root)
            store.update_lifecycle_state(
                ...
                await_context={
                    ...
                    "reason": "Need approval before REPORT",
                },
                continuity={
                    "hydrated_from_previous_session": True,
                    "next_transition": "REPORT",
                    "resume_token": "resume-await-002",
                    "workspace_root": str(workspace_root.resolve()),
                },
                current_phase="AWAIT",
                current_plan_hash="plan-await-002",
                status="awaiting",
            )
            ...
            state = write_lifecycle_transition.write_lifecycle_transition(
                ...
                phase="REPORT",
                status="active",
                active_task="report progress",
            )

            lifecycle_state = state["lifecycle_state"]
            self.assertEqual(lifecycle_state["current_phase"], "REPORT")
            self.assertEqual(lifecycle_state["status"], "active")
            self.assertEqual(lifecycle_state["active_task"], "report progress")
            self.assertIs(lifecycle_state["approval_pending"], False)
            self.assertIsNone(lifecycle_state["await_context"])
            self.assertIsNone(lifecycle_state["current_plan_hash"])
            self.assertEqual(
                lifecycle_state["continuity"]["workspace_root"],
                str(workspace_root.resolve()),
            )
            self.assertNotIn("hydrated_from_previous_session", lifecycle_state["continuity"])
            self.assertNotIn("next_transition", lifecycle_state["continuity"])
            self.assertNotIn("resume_token", lifecycle_state["continuity"])

    def test_write_lifecycle_transition_finalize_clears_active_task(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            workspace_root = Path(tmpdir)
            store = upgrade_state.UpgradeStateStore(workspace_root)
            store.update_lifecycle_state(
                active_task="ship lifecycle writer",
                approval_pending=True,
                await_context={"reason": "stale"},
                continuity={"next_transition": "REPORT"},
                current_phase="EXECUTE",
                current_plan_hash="plan-finalize",
                status="active",
            )
            ...
            lifecycle_state = state["lifecycle_state"]
            self.assertEqual(lifecycle_state["current_phase"], "FINALIZE")
            self.assertEqual(lifecycle_state["status"], "idle")
            self.assertIsNone(lifecycle_state["active_task"])
            self.assertIs(lifecycle_state["approval_pending"], False)
            self.assertIsNone(lifecycle_state["await_context"])
            self.assertIsNone(lifecycle_state["current_plan_hash"])
            self.assertEqual(lifecycle_state["continuity"]["last_terminal_status"], "completed")
            self.assertNotIn("next_transition", lifecycle_state["continuity"])

    def test_write_lifecycle_transition_non_terminal_phase_clears_terminal_status(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            workspace_root = Path(tmpdir)
            store = upgrade_state.UpgradeStateStore(workspace_root)
            store.update_lifecycle_state(
            ...
            state = write_lifecycle_transition.write_lifecycle_transition(
                ...
                phase="PLAN",
                status="active",
                active_task="new plan after terminal state",
                plan_hash="plan-after-finalize",
            )

            lifecycle_state = state["lifecycle_state"]
            self.assertEqual(lifecycle_state["current_phase"], "PLAN")
            self.assertEqual(lifecycle_state["status"], "active")
            self.assertEqual(lifecycle_state["active_task"], "new plan after terminal state")
            self.assertEqual(lifecycle_state["current_plan_hash"], "plan-after-finalize")
            self.assertEqual(
                lifecycle_state["continuity"]["workspace_root"],
                str(workspace_root.resolve()),
            )
            self.assertNotIn("last_terminal_status", lifecycle_state["continuity"])

    def test_write_lifecycle_transition_rejects_invalid_phase(self) -> None:
        writer_script = REPO_ROOT / "scripts" / "write_lifecycle_transition.py"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("invalid phase 'INVALID'", result.stderr)
    ...
    def test_orchestrator_agent_matches_thin_contract_guard(self) -> None:
        ...
        self.assertIn("write_lifecycle_transition.py", content)
    ...
    def test_session_end_triggers_cleanup_when_workspace_in_payload(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            stored = self._read_state_file(workspace_root)
            cleanup_state = stored["runtime_state"]["last_runtime_cleanup"]
            self.assertIsNotNone(cleanup_state)
            self.assertFalse(cleanup_state["dry_run"])
