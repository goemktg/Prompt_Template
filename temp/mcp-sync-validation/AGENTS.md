...
Current implementation order: lifecycle first, mediator later, council later if needed.
...
| :--- | :--- | :--- | :--- |
| **Complex orchestration** | `@orchestrator` | Lifecycle coordinator for the orchestrator-first main session. It owns coordination boundaries across `INIT`, `ATOMIZE`, `PLAN`, `REPORT`, `AWAIT`, and `FINALIZE`, while delegated specialist subagents perform substantive `EXECUTE` work. When called from within delegated execution, it provides additional planning support without crossing into specialist implementation. Mediator or council-style coordination is not part of the current baseline. | "plan this", "multi-phase task", "coordinate agents" |
...
Mediator concepts and council-style coordination remain deferred until the lifecycle-first baseline is stable enough to justify additional coordination layers.
